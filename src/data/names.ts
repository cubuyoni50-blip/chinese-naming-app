...
// 生成名字
let generatedNames: NameItem[] = [...curatedNames];
...
while (generatedNames.length < targetCount) {
  ...
  // 检查是否已存在
  if (generatedNames.some(item => item.name === name)) continue;
...
export const names: NameItem[] = generatedNames;

// 导出统计信息
console.log(`已生成 ${names.length} 个名字，每个名字都有独特的寓意解析。`);